...
/**
 * @author Jose Eldhose User Role Request DTO class
 */
@Getter
@Setter
@Validated
public class UserRolesRequestDTO {
	...
	@NotBlank(message = "Is Active must not be empty")
	...
	@NotBlank(message = "Date created must not be empty")
	...
	public UserRoles toUserRolesRequestModel(UserRolesRequestDTO request) {
		...
		try {
			...
			if (request.code != null) {
				userRoles.setCode(request.getCode());
			}
			if (request.name != null) {
				userRoles.setName(request.getName());
			}
			if (request.createdBy != null) {
				userRoles.setCreatedBy(request.getCreatedBy());
			}
			if (request.dateCreated != null) {
				userRoles.setDateCreated(request.getDateCreated());
			}
			if (request.updateBy != null) {
				userRoles.setUpdateBy(request.getUpdateBy());
			}
			if (request.dateUpdated != null) {
				userRoles.setDateUpdated(request.getDateUpdated());
			}
			if (request.isVisible != null) {
				userRoles.setIsVisible(request.getIsVisible());
			}
			if (request.isActive != null) {
				userRoles.setIsActive(request.getIsActive());
			}
			if (request.sysDefined != null) {
				userRoles.setSysDefined(request.getSysDefined());
			}
